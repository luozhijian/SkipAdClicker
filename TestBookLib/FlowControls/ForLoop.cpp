...
ForLoop::ForLoop(int line_number, const std::string& text)
    : action::BlockAction(line_number, text)
{
}

void ForLoop::Parse(const std::vector<std::string>& parts)
{
    if (parts.size() < 2) {
        throw TestException("Format error in line " + std::to_string(line_number) + ": " + action_text);
    }
    loop_variable_ = parts[1];
    sleep_seconds_ = parts.size() > 2 ? AnyToDouble(parts[2]).value_or(-1.0) : -1.0;
    total_seconds_ = parts.size() > 3 ? AnyToDouble(parts[3]).value_or(-1.0) : -1.0;
}

void ForLoop::PlayActions(const ActionRunner& runner)
{
    ...
    while (max_count.has_value() && loop < max_count.value()) {
        ++loop;
        try {
